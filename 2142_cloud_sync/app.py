...
# Route for certified translation services page
@app.route('/certified-translation')
def certified_translation():
    current_lang = get_language()
    conn = get_db_connection()
    sections = conn.execute('''
        SELECT * FROM page_sections 
    ...
    ''').fetchall()
    conn.close()
    return render_template('certified_translation.html', current_lang=current_lang, languages=SUPPORTED_LANGUAGES, sections=sections)

# Route to search for translated document by code
@app.route('/certified-translation/search', methods=['POST'])
def search_translated_document():
    current_lang = get_language()
    document_code = request.form.get('document_code', '').strip().upper()
    
    if not document_code:
        if current_lang == 'ar':
            flash('الرجاء إدخال كود الوثيقة', 'error')
        else:
            flash('Please enter document code', 'error')
        return redirect(url_for('certified_translation'))
    
    conn = get_db_connection()
    document = conn.execute(
        'SELECT * FROM translated_documents WHERE document_code = ?',
        (document_code,)
    ).fetchone()
    sections = conn.execute('''
        SELECT * FROM page_sections 
    ...
    if document:
        return render_template('certified_translation.html', 
        ...
        if current_lang == 'ar':
            flash('لم يتم العثور على وثيقة بهذا الكود', 'error')
        else:
            flash('Document not found with this code', 'error')
        return redirect(url_for('certified_translation'))

# Route to download translated document
@app.route('/certified-translation/download/<path:filename>')
def download_translated_document(filename):
    try:
...
DASHBOARD_PASSWORD = 'elegant@2020'
...
def dashboard_login():
    ...
    return render_template('dashboard_login.html', current_lang=current_lang, languages=SUPPORTED_LANGUAGES)
...
# ============================================================================
# TRANSLATED DOCUMENTS MANAGEMENT ROUTES
# ============================================================================
...
def dashboard_documents():
    conn = get_db_connection()
    documents = conn.execute('SELECT * FROM translated_documents ORDER BY uploaded_at DESC').fetchall()
    conn.close()
    current_lang = get_language()
    return render_template('dashboard_documents.html', documents=documents, current_lang=current_lang, languages=SUPPORTED_LANGUAGES)

@app.route('/dashboard/documents/upload', methods=['GET', 'POST'])
@login_required
def upload_translated_document():
    current_lang = get_language()
    ...
    if request.method == 'POST':
        ...
        if not document_code:
            if current_lang == 'ar':
                ...
                flash('Please enter document code', 'error')
            return redirect(url_for('upload_translated_document'))
        
        if not file or file.filename == '':
            if current_lang == 'ar':
                flash('الرجاء اختيار ملف للرفع', 'error')
            else:
        ...
        if not allowed_file(file.filename, 'document'):
            if current_lang == 'ar':
                flash('نوع الملف غير مدعوم. الصيغ المدعومة: PDF, DOC, DOCX, TXT, RTF', 'error')
            else:
                flash('File type not supported. Supported formats: PDF, DOC, DOCX, TXT, RTF', 'error')
            return redirect(url_for('upload_translated_document'))
        
        # Check if code already exists
        conn = get_db_connection()
        existing = conn.execute(
            'SELECT id FROM translated_documents WHERE document_code = ?',
            (document_code,)
        ).fetchone()
        ...
        if existing:
            conn.close()
            if current_lang == 'ar':
                flash('كود الوثيقة موجود بالفعل. الرجاء استخدام كود آخر', 'error')
            else:
                flash('Document code already exists. Please use a different code', 'error')
            return redirect(url_for('upload_translated_document'))
        ...
        # Save to database
        conn.execute('''
            INSERT INTO translated_documents (document_code, file_path, original_filename)
            VALUES (?, ?, ?)
        ''', (document_code, filename, original_filename))
        conn.commit()
        conn.close()
        ...
        return redirect(url_for('dashboard_documents'))
    
    return render_template('upload_document.html', current_lang=current_lang, languages=SUPPORTED_LANGUAGES)

@app.route('/dashboard/documents/delete/<int:doc_id>')
@login_required
def delete_translated_document(doc_id):
    conn = get_db_connection()
    document = conn.execute('SELECT * FROM translated_documents WHERE id = ?', (doc_id,)).fetchone()
    ...
    if document:
        ...
        conn.execute('DELETE FROM translated_documents WHERE id = ?', (doc_id,))
        conn.commit()
...
def page_not_found(e):
    # For favicon.ico and other static files, return 404 without rendering
    if request.path.startswith('/favicon.ico') or request.path.startswith('/static/'):
        return '', 404
    
    # For other 404 errors, redirect to home page with proper context
    current_lang = get_language()
    conn = get_db_connection()
    sections = conn.execute('''
        SELECT * FROM page_sections 
        ...
        ORDER BY order_index ASC
    ''').fetchall()
    stories = conn.execute('SELECT * FROM stories ORDER BY created_at DESC LIMIT 3').fetchall()
    conn.close()
    return render_template('index.html', stories=stories, sections=sections, current_lang=current_lang, languages=SUPPORTED_LANGUAGES), 404
